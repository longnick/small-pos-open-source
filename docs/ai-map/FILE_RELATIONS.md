...
- `src/components/pos/MenuGrid.test.tsx` — ordering, availability, search, empty/no-result `role=status` names, category `aria-pressed`, category switch/removal, VND, and presentation-only button coverage.
