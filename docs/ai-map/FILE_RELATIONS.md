...
- `src/components/pos/MenuManagement.tsx` — local search + category filter over `INITIAL_MENU_ITEMS`; filter chips expose `aria-pressed`; icon pencil/trash named `Sửa {item}` / `Xóa {item}`.
- `src/components/pos/MenuManagement.test.tsx` — default pressed, unpressed siblings, click exclusivity, named pencil/trash.
