...
export function StaffManagement() {
  ...
  return <div className="flex h-full flex-col gap-3">
    ...
    <ScrollArea className="-mx-1 flex-1 px-1"><div className="grid gap-2 lg:grid-cols-2">{visible.map((staff) => <div key={staff.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-secondary text-sm font-bold text-secondary-foreground">{staff.name.split(" ").slice(-1)[0]?.[0] ?? "?"}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate font-semibold text-card-foreground">{staff.name}</p><Badge className={staff.status === "Đang làm" ? "bg-success/15 text-espresso text-[10px]" : "bg-secondary text-muted-foreground text-[10px]"}>{staff.status}</Badge></div><p className="text-xs text-muted-foreground">{staff.role} · {staff.shift} · {staff.hoursThisWeek}h/tuần</p><p className="mt-0.5 inline-flex items-center gap-1 text-xs text-muted-foreground"><Phone className="h-3 w-3" />{staff.phone || "—"}</p><p className="mt-0.5 inline-flex items-center gap-1 text-xs font-medium text-primary"><Clock className="h-3 w-3" />Vào ca lúc 08:00</p></div><div className="flex flex-col gap-1"><Button variant="default" size="sm" className="h-7 px-2 text-[11px] bg-primary text-primary-foreground" onClick={() => undefined}><LogIn className="mr-1 h-3 w-3" />Xem mẫu</Button><button type="button" aria-label={`Xóa ${staff.name}`} className="flex h-7 items-center justify-center rounded-md text-muted-foreground hover:bg-destructive/10 hover:text-destructive" onClick={() => undefined}><Trash2 className="h-3.5 w-3.5" aria-hidden="true" /></button></div></div>)}</div><div className="mt-4"><h3 className="mb-2 text-sm font-bold text-foreground">Lịch sử chấm công</h3><p className="rounded-xl border border-dashed border-border p-4 text-center text-xs text-muted-foreground">Dữ liệu minh họa, không lưu chấm công.</p></div></ScrollArea>
  </div>;
}
