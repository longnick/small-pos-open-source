...
export function StaffManagement() {
  ...
  return <div className="flex h-full flex-col gap-3">
    ...
    <div className="flex flex-wrap gap-2">{["all", ...SHIFTS].map((shift) => <button key={shift} onClick={() => setShiftFilter(shift)} className={`whitespace-nowrap rounded-full px-3 py-1.5 text-sm font-medium transition-colors ${shiftFilter === shift ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>{shift === "all" ? "Tất cả ca" : shift}</button>)}</div>
