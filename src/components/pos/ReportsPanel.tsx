...
export function ReportsPanel() {
  ...
  return <div className="flex h-full flex-col gap-3">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-bold text-foreground">Báo cáo</h2><p className="text-xs text-muted-foreground">Tổng quan kinh doanh</p></div><div className="flex gap-2">{RANGES.map((item) => <button key={item} onClick={() => setRange(item)} className={`whitespace-nowrap rounded-full px-3 py-1.5 text-sm font-medium transition-colors ${range === item ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>{item}</button>)}</div></div>
